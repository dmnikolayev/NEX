(() => {
  ...
  function fmtPower(watts, latin = false) {
    ...
    return value >= 1000
      ? `${(value / 1000).toFixed(1)} ${unitKw}`
      : `${Math.round(value)} ${unitW}`;
  }

  function announceImportantChange(previous, current) {
    if (!previous) return;

    if (previous.grid.online && !current.grid.online) {
      houseSpirit.say("⚡ Світло пішло. Будинок уже працює від батареї.");
      return;
    }

    if (!previous.grid.online && current.grid.online) {
      houseSpirit.say("⚡ Мережа повернулася. Напруга знову стабільна.");
      return;
    }

    if (previous.internet.online && !current.internet.online) {
      houseSpirit.say("📡 Космос сьогодні мовчить. Starlink недоступний.");
      return;
    }

    if (!previous.internet.online && current.internet.online) {
      houseSpirit.say("📡 Космос знову відповідає.");
      return;
    }

    if (previous.solar.power < 100 && current.solar.power >= 100) {
      houseSpirit.say("🌞 Сонце прокинулося. Панелі почали генерацію.");
    }
  }
  ...
  function render(state) {
    ...
    announceImportantChange(previousState, state);
    previousState = state;
  }
  ...
  function startDemo() {
    setInterval(() => {
      ...
      const solarPower = Math.max(0, current.solar.power + (Math.random() - 0.48) * 180);
      const housePower = Math.max(180, current.house.power + (Math.random() - 0.5) * 90);

      HouseState.set({
        solar: { power: solarPower },
        inverter: { power: Math.max(housePower, solarPower) },
  ...
  setTimeout(() => {
    houseSpirit.say("🌞 Будинок прокинувся. Поки все спокійно.", { duration: 5500 });
  }, 700);
